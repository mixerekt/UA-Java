...
public class Client {
     ...
     * @param requestedSessionTimeout requested session time out (in ms) - if null, use 3600000 (one hour)
     * @param sessionName             session name - if null a random GUID is used to generate the
     *                                name
     * @return the session
    ...
    public Session createSession(SecureChannel channel,
        ...
        log.debug("MessageSecurityMode: {}", channel.getMessageSecurityMode());
        if (!MessageSecurityMode.None.equals(channel.getMessageSecurityMode())) {
            ...
            log.debug("Algorithm: {}", signatureAlgorithm);
        ...
        if (filteredEndpoints.length == 0) {
            log.error(
                    ...
                    endpoint);
            log.debug("endpoints={}", Arrays.asList(endpoints));
            throw new ServiceResultException(
...
//		}
    ...
    private void validateEndpoints(String uri, EndpointDescription[] endpoints,
         ...
         */
        List<EndpointDescription> dedl = new ArrayList<>();
        for (EndpointDescription e : discoveredEndpoints) {
            EndpointDescription ne = EndpointDescription.newInstanceFrom(e);
            ne.setServer(null);
            ne.setServerCertificate(null);
            dedl.add(ne);
        }
        List<EndpointDescription> edl = new ArrayList<>();
        ...
        for (EndpointDescription e : dedl) {
            if (!edl.contains(e)) {
                log.error("The endpoint received from GetEndpoints is not in the endpoints of CreateSessionResponse. Endpoint={}", e);
                log.error("GetEndpoints returned endpoints={}", dedl);
                log.error("CreateSessionResponse endpoints={}", edl);
