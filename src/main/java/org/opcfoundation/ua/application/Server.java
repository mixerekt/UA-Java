...
public class Server {
    ...
    public void addServiceHandler(Object serviceHandler) {
        ...
        log.debug("addServiceHandler: {}", serviceHandler);
        log.trace("addServiceHandler: from {}", Thread.currentThread().getStackTrace()[2]);
    ...
    public void close() {
        for (EndpointHandle handle : boundHandles) {
            log.debug("unbind: {}", handle);
            handle.close();
        }
        boundHandles.clear();
        log.info("Server {} closed", this);
        OpcTcpServer opctcpServer = getApplication().getOpctcpServer();
        HttpsServer httpsServer = getApplication().getHttpsServer();
    ...
    public EndpointDescription[] getEndpointDescriptions(SocketAddress requestAddress) {
        ...
        if (requestAddress != null) {
            ...
            log.trace("Server.getEndpointDescriptions: requestAddress=", requestAddress);
            for (EndpointServer es : endpointBindings.getEndpointServers()) {
                log.trace("Server.getEndpointDescriptions: EndpointServer es=", es);
        ...
        for (Endpoint ep : endpointBindings.getEndpointAddresses()) {
            ...
            for (KeyPair keypair : application.getApplicationInstanceCertificates()) {
                ...
                log.trace("getEndpointDescriptions: endpoint={}", ep);
                log.trace("getEndpointDescriptions: keyPair={}", keypair);
                if (log.isTraceEnabled())
                    log.trace("getEndpointDescriptions: securityModes={}", Arrays.toString(securityModes));
        ...
        log.trace("getEndpointDescriptions: result={}" + result);
